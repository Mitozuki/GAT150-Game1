...
namespace kiko
{
	...
	void Renderer::DrawTexture(Texture* texture, const Rect& source, const Transform& transform)
	{
		...
		// https://wiki.libsdl.org/SDL2/SDL_RenderCopyEx
		SDL_RenderCopyEx(m_renderer, texture->m_texture, (SDL_Rect*)(&source), &dest, RadiansToDeg(mx.GetRotation()), nullptr, SDL_RendererFlip::SDL_FLIP_NONE);
	}
	void Renderer::DrawTexture(Texture* texture, const Rect& source, const Transform& transform, const vec2& origin, bool fliph)
	{
		...
		// https://wiki.libsdl.org/SDL2/SDL_RenderCopyEx
		SDL_RenderCopyEx(m_renderer, texture->m_texture, (SDL_Rect*)(&source), &dest, RadiansToDeg(mx.GetRotation()), &center, (fliph) ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
